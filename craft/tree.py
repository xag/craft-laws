...
DECISIONS.append(Node(
    ...
    payload={
        "rationale":
            ...
            "table are gone; a count-based cap (premise nodes read as evidence lines) shipped "
            "for one hour and was refuted by the owner - one premise can quote a 254-run suite, "
            "and one quote split across two nodes is not two lines - so the machine computes no "
            "grade: a strength above limited carries `basis`, the note's own traceable account, "
            "or convicts under the practice family's IPCC-cited law; a verified entailment from "
            "given premises needs none, being necessity. craft/account_laws.py is the registry; "
            "tests/test_law_registry.py, run by CI, refuses an unregistered decider id, an "
            "uncited law, a cherry-picked census row, a decider on a judge-routed row, a dead "
            "registry entry and a missing capture. Its first run caught its own scanner "
            "miscounting law constructors as convictions, which is what an alarm is for.",
        "note":
