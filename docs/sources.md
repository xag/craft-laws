...
5. **Federal plain language guidelines** — statutory (Plain Writing Act 2010), public domain; the canonical pages now live under digital.gov and several old plainlanguage.gov paths redirect or render client-side, so fetch carefully and keep the caught text. The March 2011 document is captured at `docs/sources/plain-2011-federal-guidelines.pdf`. Census'd whole in prose (docs/practice-sources.md, 2026-08-24) and re-read as data: `python -m craft.census_plain`.
...
- **Apple HIG (Typography)** — captured later the same day from the site's own data endpoint (tutorials/data/design/human-interface-guidelines/typography.json) after the rendered page resisted fetching: the legibility sentence and the per-platform minimum-size table (iOS/iPadOS 11 pt). Roots `type-stays-legible`, whose falsifier already measured against that floor. The general exclusion of the HIG as a mined source stands; this is the cite-where-it-is-the-root case the exclusion note always allowed.
