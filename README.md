...
- **The rulings** — `craft/rulings.py`: standing convictions carded into judgeable questions for the app's owner, whose verdicts — stand, exempt, fix — are recorded and consulted by every check, never silently. The pipeline reads a drawing, findings, and a rulings file; the app brings only its authored question texts and its grouping of findings into decisions.
...
- **The practice** — `craft/practice.py`, rendered at the end of [`LAWS.md`](LAWS.md): laws about the *work* rather than the interface, same shape, separate gate. An interface law fires on a screen; a practice law fires on a claim — an assertion like "it is done" or "this fixes it". `craft/claims.py` holds their deciders, which read a repo's `claims.jsonl`: one JSON record per claim, filed at the moment of claiming, with its evidence classed as user-surface, stand-in, or producer. CI runs the deciders wherever that file exists, so a claim nobody could stand behind cannot merge. The protocol for filing lives in the user-level `claim` skill, not here — a repo teaches what its records mean, a skill teaches when to write one.
