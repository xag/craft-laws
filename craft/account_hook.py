...
INSTRUCTION = """\
craft.account is ON for this session. Before you answer, decide whether this turn
ARGUES for anything -- a conclusion, a diagnosis, a recommendation, a judgement about
what is true. If it does, file the argument as data alongside the answer:

  .craft/accounts/<session-id>/<n>.json   in the repo you are working in

  {"nodes": [
     {"id": "p1", "type": "I", "ground": "producer|stand-in|user-surface|given",
      "text": "<a premise, quoted from what you actually observed>"},
...
The scheme decides how strongly you may state the conclusion: deduction licenses
robust, verified-source medium, sign/example/authority limited, absence nothing at all.
A count of occurrences is a SIGN. Naming it a proof is the failure this exists to catch.

A turn that argues nothing -- a yes, a status line, a file written to order -- files
nothing, and that is the honest state, not a gap."""
...
def stop(payload: dict) -> int:
    ...
    if not files:
        return 0            # nothing filed: the silence note is the caller's, below
