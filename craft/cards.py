...
# A CARD'S SELF-CONTRADICTION HAS NO DECIDER, and this note is why the one removed on
# 2026-08-22 should not return. It paired two word lists -- a claim matching
# /says nothing|no message|nothing happens|in silence/ against evidence matching
# /already taken|please fill|required|says so|error|refus/ -- and concluded that the
# question contradicted its own evidence. Neither list is the subject of
# what-accompanies-a-claim-supports-it. Whether an accompaniment supports a claim is
# meaning, and two independent matches over different text is a guess about meaning
# wearing a regex: a card saying an EMPTY STATE says nothing, beside a finding whose
# `why` happens to contain 'refuses', convicts on nothing at all. The evidence blob is
# every finding's quote and why joined, so the two halves need not even be about the
# same element.
#
# The law keeps its other decider, check_pictures_show_the_defect, which asks a
# structural question about declared fields and does not read for sense. The
# contradiction half is read by a person. See a-word-list-is-a-reading-not-a-mechanization.
